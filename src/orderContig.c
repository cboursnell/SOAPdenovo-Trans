...
static void createAnalogousCnt (unsigned int sourceStart, CONNECT * originCnt, int gap, unsigned int targetStart, unsigned int targetStop)
{
	CONNECT *temp_cnt;
...
static void add1LongPEcov (unsigned int fromCtg, unsigned int toCtg, int weight)
{
	...
	if (contig_array[fromCtg].from_vt != contig_array[toCtg].from_vt || contig_array[fromCtg].to_vt != contig_array[toCtg].to_vt)
	{
...
static void downSlide ()
{
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		while (ite_cnt)
		{
			if (ite_cnt->deleted || ite_cnt->mask || ite_cnt->singleInScaf || ite_cnt->nextInScaf || ite_cnt->prevInScaf || ite_cnt->inherit)
			{
				ite_cnt = ite_cnt->next;
				continue;
			}
...
static CONNECT *getNextContig (unsigned int ctg, CONNECT * preCNT, boolean * exception)
{
	...
	*exception = 0;
	...
	if (preCNT && preCNT->nextInScaf)
	{
		...
		{
			printf ("getNextContig: arc(%d %d) twin (%d %d) with mask %d deleted %d\n", ctg, nextCtg, getTwinCtg (nextCtg), getTwinCtg (ctg), cn_temp->mask, cn_temp->deleted);
			...
			cn_temp = getCntBetween (getTwinCtg (nextCtg), getTwinCtg (ctg));
...
static void deleteWeakCnt (int cut_off)
{
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		while (cn_temp1)
		{
			if (!cn_temp1->mask && !cn_temp1->deleted && !cn_temp1->nextInScaf && !cn_temp1->singleInScaf && !cn_temp1->prevInScaf)
			{
				counter++;
			}
			...
			else if (!cn_temp1->deleted && cn_temp1->weight > 0 && cn_temp1->weight < cut_off && !cn_temp1->nextInScaf && !cn_temp1->prevInScaf)
			{
				cn_temp1->deleted = 1;
				cn_temp1->weak = 1;
	...
	while (flag)
	{
		...
		for (i = 1; i <= num_ctg; i++)
		{
			...
			else
			{
				...
				cn1 = getCntBetween (*(unsigned int *) darrayGet (solidArray, solidCounter - 2), *(unsigned int *) darrayGet (solidArray, solidCounter - 1));
				...
				if (cn2->prevInScaf)
					substitueDSinScaf (cn2, *(unsigned int *) darrayGet (solidArray, 0), *(unsigned int *) darrayGet (solidArray, 1));
...
boolean checkOverlapInBetween (double tolerance)
{
	...
	{
		gap = nodeDistanceInOrder[i + 1] - nodeDistanceInOrder[i] - contig_array[nodesInSubInOrder[i + 1]].length;
...
static void recoverMask ()
{
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		*(unsigned int *) darrayPut (scaf5, num5++) = i;
		...
		{
			*(unsigned int *) darrayPut (solidArray, solidCounter++) = *(unsigned int *) darrayGet (tempArray, t);
		...
		*(unsigned int *) darrayPut (solidArray, solidCounter++) = *(unsigned int *) darrayGet (tempArray, tempCounter - 1);
...
static void freezing ()
{
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		*(unsigned int *) darrayPut (scaf5, num5++) = i;
		...
		for (j = num3 - 1; j >= 0; j--)
			*(unsigned int *) darrayPut (solidArray, solidCounter++) = *(unsigned int *) darrayGet (scaf3, j);

		for (j = 0; j < num5; j++)
			*(unsigned int *) darrayPut (solidArray, solidCounter++) = *(unsigned int *) darrayGet (scaf5, j);
		...
		if (firstCtg == 0 || lastCtg == 0)
		{
			...
			for (j = 0; j < solidCounter; j++)
				printf ("->%d(%d %d)", *(unsigned int *) darrayGet (solidArray, j), contig_array[*(unsigned int *) darrayGet (solidArray, j)].mask,
...
void ScafStat ( int len_cut, char * graphfile )     
{
	...
	//*********************** Contig ******************************
		...
		fprintf(fo,"Number_of_contigs_in_scaffolds(Singleton)\t%ld\n",Singleton_Number_Scaf);
		fprintf(fo,"Number_of_contigs_not_in_scaffolds\t%ld\n",Scaffold_Number_Contig - Singleton_Number_Scaf);	
		fprintf(fo,"Average_number_of_contigs_per_scaffold\t%.1f\n", 1.0 * Scaffold_Number_Contig / Scaffold_Number_Scaf);	
...
void RPKMStat (char * graphfile )     
{
    ...
    sprintf ( line, "%s.RPKM.Stat", graphfile );
    fo = ckopen ( line, "w" );
    ...
    //printf("Scaffold_Num=%d   Singleton_Num=%d\n", Scaffold_Num,Singleton_Num);  
...
//***************************************************
    ...
    int  *S_Length = (int *)malloc(sizeof(int)* (Scaffold_Num+1));
	int  *C_Length = (int *)malloc(sizeof(int)* (Singleton_Num+1));
    char type = 'N';
	int index = 0;
	int  len = 0;

    for(i=0;i<=Scaffold_Num;i++)
		S_Length[i]=0;
	for(i=0;i<=Singleton_Num;i++)
		C_Length[i]=0;
    ...
    //for(index=0;index<=Scaffold_Num;index++)  printf(">scaffold%d\t%d\n",index,S_Length[index]);
	//for(index=0;index<=Singleton_Num;index++) printf(">C%d\t%d\n",index,C_Length[index]);

    fclose ( fp );

//***************************************************
    sprintf ( line, "%s.peGrads", graphfile );
    fp3 = ckopen ( line, "r" );

    int Reads_Num = 0;
	fscanf(fp3, "%s %d %d", buf, &i, &Reads_Num);
	
	//printf("Reads_Num=%d\n", Reads_Num); 
    fclose (fp3);

//***************************************************
	sprintf ( line, "%s.readOnScaf", graphfile );
	fp2 = ckopen ( line, "r" );

    char ch_temp[40];
    int  *Reads = (int *)malloc(sizeof(int)* (Reads_Num+1));
    for(i=0;i<=Reads_Num;i++)
		Reads[i]=0;
  
	while (!feof(fp2))
	{
      fscanf(fp2, "%s ", ch_temp);
      if(ch_temp[0] == '>')
      {
        //printf("%s\n",ch_temp);
        if(ch_temp[1] == 's')
			fgets(buf,10000000,fp2);
      }
	  else
	  {
	    i=atoi(ch_temp);
...
//***************************************************
    ...
    int  *S_hits = (int *)malloc(sizeof(int)* (Scaffold_Num+1));
	int  *C_hits = (int *)malloc(sizeof(int)* (Singleton_Num+1));    
    int  uniq_reads_num = 0;
	...
	while (!feof(fp2))
	{
      ...
      if(ch_temp[0] == '>')
      {
        Nucleotide = ch_temp[1];

        if(Nucleotide == 's')
        {
		  type = 'S';
		  sscanf(ch_temp, "%9s%d",buf,&m);		  
		  index = m; 	
		  fgets(buf,10000000,fp2);
        }
		else if(Nucleotide == 'C')	
	  ...
	  else
	  {
		...
		{ 
		  ...
		  //printf("%d\t%d\t%d\t%d\n",i,Reads[i],S_hits[index],C_hits[index]);
		}
		fgets(buf,10000000,fp2);
	  }
	...
	//Output
	fprintf(fo,"# Notice:RPKM calculation base on K-mer mapping.\n");
	fprintf(fo,"# Total_unique_reads_num=%d\n",uniq_reads_num);
	fprintf(fo,"Transcript_ID\tLength\tUniq_reads_num\tRPKM\n");
	...
	{	  
      if(S_Length[i] != 0)
      {
        rpkm= (S_hits[i]*(1e+9))/(1.0*uniq_reads_num*S_Length[i]);
        fprintf (fo, "scaffold%d\t%d\t%d\t%f\n", i,S_Length[i],S_hits[i],rpkm);
        //fprintf (fo, "scaffold%d\t%d\t%f\n", i,S_hits[i],rpkm);
      }
	}
	...
	{	  
      if(C_Length[i] != 0)
      {
        rpkm= (C_hits[i]*(1e+9))/(1.0*uniq_reads_num*C_Length[i]);
        fprintf (fo, "C%d\t%d\t%d\t%f\n", i,C_Length[i],C_hits[i],rpkm);
        //fprintf (fo, "C%d\t%d\t%f\n", i,C_hits[i],rpkm);
      }
	}

	printf("Output assembly RPKM statistics finished! \n ");
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		*(unsigned int *) darrayPut (scaf5, num5++) = i;
		...
		for (j = num3 - 1; j >= 0; j--)
		{
			...
			{
				fprintf (fp, "%-10d %-10d +   %d ", index_array[*(unsigned int *) darrayGet (scaf3, j)], len, contig_array[*(unsigned int *) darrayGet (scaf3, j)].length + overlaplen);
			...
			{
				fprintf (fp, "%-10d %-10d -   %d ", index_array[getTwinCtg (*(unsigned int *) darrayGet (scaf3, j))], len, contig_array[*(unsigned int *) darrayGet (scaf3,
							j)].length + overlaplen);
				weak = printCnts (fp, *(unsigned int *) darrayGet (scaf3, j));
				/*
				   if(weak)
			...
			{
				num_route = num_trace = 0;
				traceAlongArc (*(unsigned int *) darrayGet (scaf3, j), prev_ctg, max_steps, gap - ins_size_var, gap + ins_size_var, 0, 0, &num_route);
		...
		for (j = 0; j < num5; j++)
		{
			...
			{
				fprintf (fp, "%-10d %-10d +   %d ", index_array[*(unsigned int *) darrayGet (scaf5, j)], len, contig_array[*(unsigned int *) darrayGet (scaf5, j)].length + overlaplen);
			...
			{
				fprintf (fp, "%-10d %-10d -   %d ", index_array[getTwinCtg (*(unsigned int *) darrayGet (scaf5, j))], len, contig_array[*(unsigned int *) darrayGet (scaf5,
							j)].length + overlaplen);
				weak = printCnts (fp, *(unsigned int *) darrayGet (scaf5, j));
				/*
				   if(weak)
			...
			{
				num_route = num_trace = 0;
				traceAlongArc (*(unsigned int *) darrayGet (scaf5, j), prev_ctg, max_steps, gap - ins_size_var, gap + ins_size_var, 0, 0, &num_route);
...
static int inputLinks (FILE * fp, int insertS, char *line)
{
	...
	if (strlen (line))
	{
		...
		if (1)
		{
			...
			if(bal_ctg == ctg ||bal_toCtg==toCtg ){				
				add1Connect (ctg, toCtg, gap, wt, 0);
				add1Connect (bal_toCtg, bal_ctg, gap, wt, 0);
			
				connection=getCntBetween(ctg, toCtg);
				connection->PECount = wt;
				connection=getCntBetween(bal_toCtg, bal_ctg);
				connection->PECount = wt;
				...
				if (insertS > 1000 && contig_array[ctg].from_vt == contig_array[toCtg].from_vt &&	// on the same scaff
...
static boolean dispatch1node (int dis, unsigned int tempNode, int maxNodes, FibHeap * dheap, FibHeap * uheap, int *DmaxDis, int *UmaxDis)
{
	boolean ret;
...
static boolean workOnDheap (FibHeap * dheap, FibHeap * uheap, boolean * Dwait, boolean * Uwait, int *DmaxDis, int *UmaxDis, int maxNodes)
{
	if (*Dwait)
	{
		return 1;
	...
	while ((indexInArray = removeNextNodeFromHeap (dheap)) != 0)
	{
		...
		while (us_cnt)
		{
			if (us_cnt->deleted || us_cnt->mask || contig_array[getTwinCtg (us_cnt->contigID)].inSubGraph)
			{
				us_cnt = us_cnt->next;
				continue;
			}
...
static boolean workOnUheap (FibHeap * dheap, FibHeap * uheap, boolean * Dwait, boolean * Uwait, int *DmaxDis, int *UmaxDis, int maxNodes)
{
	if (*Uwait)
	{
		return 1;
...
static void arrangeNodes_general ()
{
	...
	for (i = 1; i < nodeCounter; i++)
	{
		...
		gap = ctg4heapArray[i + 1].dis - ctg4heapArray[i].dis - contig_array[node2].length;
	...
	if (ite_cnt)
	{
		...
		next_cnt = checkConnect (getTwinCtg (ctg4heapArray[nodeCounter].ctgID), getTwinCtg (ctg4heapArray[nodeCounter - 1].ctgID));
...
boolean checkOverlapInBetween_general (double tolerance)
{
	...
	for (i = 1; i < nodeCounter; i++)
	{
		gap = ctg4heapArray[i + 1].dis - ctg4heapArray[i].dis - contig_array[ctg4heapArray[i + 1].ctgID].length;
...
static void setBreakPoints (DARRAY * ctgArray, int count, int weakest, int *start, int *finish)
{
	int index = weakest - 1;
	...
	for (i = 1; i <= num_ctg; i++)
	{
		...
		*(unsigned int *) darrayPut (scaf5, num5++) = i;
		...
		for (t = 1; t < tempCounter; t++)
		{
			...
			if (contig_array[prevCtg].from_vt != contig_array[thisCtg].from_vt || contig_array[prevCtg].indexInScaf > contig_array[thisCtg].indexInScaf)
			{
				prevCtg = thisCtg;
				continue;
			}
...
static void checkCircle ()
{
	...
	printf ("%d loops found and removed in Graph\n", counter);
}
